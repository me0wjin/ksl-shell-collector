...
def show_preview(cap, width, height, warmup):
    ...
    while True:
        ...
        draw_center_text(frame, "Camera Ready", y_offset=-45, scale=1.1, color=(0, 255, 255))
        draw_center_text(frame, f"Countdown starts in {remaining}", y_offset=25, scale=0.9, color=(255, 255, 255))
        ...
        show_frame("KSL Recorder", frame, display_scale=2)
...
def run_countdown(cap, width, height, countdown):
    ...
    while True:
        ...
        draw_center_text(frame, f"Recording starts in {remaining}", y_offset=-20, scale=1.1, color=(0, 255, 255))
        draw_center_text(frame, "Get ready!", y_offset=40, scale=1.0, color=(255, 255, 255))
...
def record_video(output_path, camera_index=0, duration=8, countdown=3, warmup=3, width=640, height=480, fps=20):
    ...
    show_preview(cap, width, height, warmup)

    # 2. 녹화 시작 전 카운트다운을 표시한다.
    run_countdown(cap, width, height, countdown)
